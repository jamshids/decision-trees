...
def minimize_KDE_entropy(dat, labels, kernel_CDF, priors):
    ...
    (a, b) = (dat.min(), dat.max())
    (J_a, J_b) = (OBJ_EVAL(dat, labels, a, kernel_CDF, priors), 
                  OBJ_EVAL(dat, labels, b, kernel_CDF, priors))
    ...
    while(np.abs(a-b) > tol) and (cnt<max_iter):
        ...
        x_l = (a + x_m) / 2.
        x_r = (b + x_m) / 2.
        J_r, J_l = (OBJ_EVAL(dat, labels, x_r, kernel_CDF, priors), 
                    OBJ_EVAL(dat, labels, x_l, kernel_CDF, priors))
